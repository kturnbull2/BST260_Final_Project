...
p <- tidy_df %>%
   filter(measure == "cases", state %in% levels(factor(sort(tidy_df$state)))[1:5]) %>%
...
```{r}
# extra code that I wrote but didn't use (but may need in the future)
...
# join population data to covid data frame
covid <- covid %>% left_join(population, by = "state")

# create per 10 million population variables for cases and deaths
covid <- covid %>% 
  mutate(new_cases_7dayavg_per_cap = new_cases_7dayavg / (population / 10000000),
         new_deaths_7dayavg_per_cap = new_deaths_7dayavg / (population / 10000000))

# join combined covid and population data with policies data
df <- covid %>% left_join(policies, by = c("state", "date"))
```
