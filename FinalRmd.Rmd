...
[simplemaps](https://simplemaps.com/data/us-cities). A dataset connecting states to their two letter abbreviation was also used:
[link](https://worldpopulationreview.com/states/state-abbreviations). A list of Trump 2020 election rallies was web-scraped from
[Wikipedia](https://en.wikipedia.org/wiki/List_of_post-2016_election_Donald_Trump_rallies). 
...
From the base model, it appears that nearly all of the variables are associated with log(cases) in the training set (though, notably, grocery/pharmacy mobility change and governor's political party are not). In order to assess how well the model predicts in the test set, we calculated the RMSE and used an observed vs. predicted plot for visualization. 
...
From the Observed vs. Predicted plot above, we can see that this model under-predicts many of these values, meaning that the predicted number of cases is less than the observed number of cases. This discrepancy is also captured in the RMSE, which is quite high at 135,824.7 (to add context to the size of the RMSE, we are predicted total number of cases). With the benefit of hindsight, it does logically make sense that we would under-predict cases from 11/15 to 11/24, because there was such a massive international COVID-19 spike occurring around that time, however, we still think we can decrease that RMSE more by updating the model based on what was observed in the data exploration plots. Several more models were tested (see the un-included chunk in the .Rmd file for details), and the best model for prediction that was found is as follows:
...
As can be seen in the above Observed vs. Prediction plot, the model still under-predicts cases, however, this final model does appear to bring the values much closer to the 1:1 observed vs. predicted line. Additionally, the RMSE has decreased- it is still quite large but it is now 115,593.3 instead of 135,824.7. One interesting aspect of this final prediction model is that the mask-use data, which we anticipated would be useful for prediction, is not included. Additionally, many of the mobility change variables were also not useful for prediction, though parks mobility change definitely was. In order to try to understand why mask-use data was not useful, we created a map of the United States shaded by the percentage of people reporting wearing a mask in public on 11/19/2020, and then compared that visualization to a plot of the total number of cases per state on that same date. 
